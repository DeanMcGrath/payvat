  ...
  try {
    ...
    return NextResponse.json({
      ...
      debugInfo: {
        ...
        textLength: result.extractedData?.extractedText?.length || 0,
        contains111_36: result.extractedData?.extractedText?.includes('111.36') || false,
        contains90_85: result.extractedData?.extractedText?.includes('90.85') || false,
        containsVW: result.extractedData?.extractedText?.toLowerCase().includes('volkswagen') || 
                   result.extractedData?.extractedText?.toLowerCase().includes('vw financial') || false,
        containsTotalAmountVAT: result.extractedData?.extractedText?.toLowerCase().includes('total amount vat') || false,
