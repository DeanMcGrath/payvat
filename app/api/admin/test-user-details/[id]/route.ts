...
async function testUserDetails(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    console.log(`[${requestId}] User details query completed`)
      ...
      return NextResponse.json(
        { 
          ...
          actualError: userQueryError instanceof Error ? userQueryError.message : String(userQueryError)
        },
        { status: 500 }
      )
    }
    ...
    console.error(`[${errorId}] Critical test admin user details fetch error:`, {
...
export const GET = testUserDetails